...
exports.printColor = exports.createDir = void 0;
    ...
    handler(argv) {
        if (file) {
            ...
            if (typeof argv.usuario === 'string') {
                createDir(argv.usuario);
            }
            (0, fs_1.readFile)('notas/' + argv.usuario + '/notas.txt', (_, data) => {
                if ((typeof argv.titulo === 'string') && (typeof argv.usuario === 'string')) {
                    let is_in;
                    is_in = false;
                    n = JSON.parse(data.toString());
                    n.notas.forEach((element, i) => { if (element.titulo == argv.titulo) {
                    ...
                    try {
                        (0, fs_1.writeFileSync)('notas/' + argv.usuario + '/notas.txt', JSON.stringify(n));
                    }
                    catch (err) {
                        console.log(chalk_1.default.red('Se ha producido un error'));
                    }
            ...
            (0, fs_1.readFile)('notas/' + argv.usuario + '/notas.txt', (_, data) => {
                if ((typeof argv.titulo === 'string') && (typeof argv.data === 'string') && (typeof argv.usuario === 'string') && (typeof argv.color === 'string')) {
                    ...
                    n.notas.forEach((element) => {
                        if (element.titulo == argv.titulo) {
                            is_in = true;
                            if ((typeof argv.color === 'string') && (typeof argv.data === 'string')) {
                                element.data = argv.data;
                                element.color = argv.color;
        ...
        if (file) {
            ...
            (0, fs_1.readFile)('notas/' + argv.usuario + '/notas.txt', (_, data) => {
                if ((typeof argv.titulo === 'string') && (typeof argv.usuario === 'string')) {
                    ...
                    is_in = false;
                    n = JSON.parse(data.toString());
                    n.notas.forEach((element) => { if (element.titulo == argv.titulo) {
                        is_in = true;
